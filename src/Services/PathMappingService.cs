...
public class PathMappingService
{
    ...
    public PathMappingService(IOptionsMonitor<SettingsOptions> optionsAccessor, ILogger<PathMappingService> logger)
    {
        ...
        _optionsAccessor.OnChange(settings =>
            {
                _localPathMappings = settings.PathMappings.ToDictionary(x => x.RemotePath, x => x.LocalPath);
                _remotePathMappings = settings.PathMappings.ToDictionary(x => x.LocalPath, x => x.RemotePath);
            }
        );
    }
